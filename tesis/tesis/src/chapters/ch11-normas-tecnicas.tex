...
\begin{table}[!ht]
    ...
    \begin{tabular}{>{\raggedright\arraybackslash}p{3.2cm} >{\raggedright\arraybackslash}p{4.0cm} >{\raggedright\arraybackslash}p{7.4cm}}
        ...
        Intercambio de datos & RFC 8259 (JSON) & Estructura el contrato de datos entre la app móvil y el backend RAG mediante cargas livianas, legibles y portables. \\
...
\section{Criterios de aplicación por capa de la solución}

Para que el marco normativo no quede como una enumeración aislada, se vincula cada referencia con la capa técnica donde tiene mayor incidencia. Este criterio permite revisar el diseño desde una perspectiva de cumplimiento progresivo: primero se identifica la capa, luego el riesgo principal, después la referencia aplicable y, finalmente, la evidencia que debe quedar documentada en el prototipo.
...
\begin{table}[!ht]
    ...
    \setlength{\tabcolsep}{4pt}
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{>{\raggedright\arraybackslash}p{3.1cm} >{\raggedright\arraybackslash}p{4.0cm} >{\raggedright\arraybackslash}p{4.0cm} >{\raggedright\arraybackslash}p{5.1cm}}
        \hline
        \textbf{Capa técnica} & \textbf{Riesgo o necesidad} & \textbf{Referencia principal} & \textbf{Criterio aplicado en la tesis} \\
        \hline
        Infraestructura BLE & Interoperabilidad, identificación de sala y lectura estable del dispositivo. & Bluetooth Core Specification & Uso de advertising BLE, \textit{service data}, UUID de servicio y payload documentado. \\
        Aplicación móvil & Manejo de permisos, experiencia BYOD, persistencia local y degradación funcional. & ISO/IEC/IEEE 12207 y buenas prácticas OWASP & Separación de módulos, control de permisos, lectura local del recorrido y fallback por QR o selección manual. \\
        Backend conversacional & Entrada no confiable, trazabilidad de fuentes y reducción de respuestas arbitrarias. & OWASP API Security y OWASP LLM Applications & Validación de payload, filtros por sala/obra, recuperación con fuentes y metadatos de soporte. \\
        Corpus curatorial & Consistencia semántica, metadatos, actualización y reutilización documental. & ISO/IEC 25012, CIDOC CRM, LIDO, Dublin Core & Identificadores estables de sala y obra, metadatos mínimos y fuentes curatoriales recuperables. \\
        Datos del visitante & Minimización, finalidad y cuidado sobre sesiones, voz o interacciones. & Ley N.\ 29733 & Uso de identificadores funcionales, registro mínimo y recomendación de consentimiento y retención limitada en una fase institucional. \\
        Operación y mantenimiento & Evolución del sistema, soporte, documentación y control de cambios. & ISO/IEC/IEEE 12207 & Modularidad por subsistemas, protocolo de pruebas y anexos técnicos de funciones principales. \\
        \hline
    \end{tabular}%
    }
    \caption[Criterios de aplicación normativa por capa]{Criterios de aplicación normativa por capa técnica de la solución propuesta. Fuente: elaboración propia.}
    \label{tab:criterios-normativos-capa}
\end{table}

La Tabla~\ref{tab:criterios-normativos-capa} muestra que cada estándar cumple una función concreta dentro del diseño. En particular, la capa BLE se relaciona con interoperabilidad física; la aplicación móvil, con experiencia, permisos y continuidad; el backend RAG, con seguridad conversacional y trazabilidad; y el corpus curatorial, con calidad de datos e interoperabilidad patrimonial.
...
ISO/IEC/IEEE 12207 establece un marco de procesos para el ciclo de vida del software, incluyendo desarrollo, operación, mantenimiento y mejora~\cite{iso12207}. En la solución propuesta, esta referencia no se adopta como certificación formal, sino como criterio estructurante del proyecto. Su influencia se observa en la separación explícita de responsabilidades entre el módulo IoT BLE, la aplicación móvil y el backend RAG, así como en la delimitación de contratos entre adquisición BLE, experiencia móvil, persistencia local y backend conversacional.
...
\section{Matriz de verificación técnica mínima}

Además del alineamiento conceptual, la solución requiere criterios observables que permitan verificar si una implementación concreta respeta el diseño normativo planteado. Por ello, la Tabla~\ref{tab:matriz-verificacion-normativa} presenta una matriz mínima de control aplicable antes de un piloto institucional.
...
\begin{table}[!ht]
    ...
    \setlength{\tabcolsep}{4pt}
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{>{\raggedright\arraybackslash}p{3.6cm} >{\raggedright\arraybackslash}p{5.1cm} >{\raggedright\arraybackslash}p{4.2cm} >{\raggedright\arraybackslash}p{3.8cm}}
        \hline
        \textbf{Control verificable} & \textbf{Pregunta de revisión} & \textbf{Evidencia esperada} & \textbf{Estado en la tesis} \\
        \hline
        Payload BLE documentado & ¿La app puede interpretar sala, nodo, potencia y batería sin conexión persistente? & Estructura del \textit{service data}, UUID y decodificación en la app. & Cubierto a nivel de prototipo. \\
        Contrato HTTP/JSON estable & ¿La consulta contextual conserva pregunta, sala, obra, sesión y contexto curatorial? & Modelo de request/response y endpoint documentado. & Cubierto a nivel de prototipo. \\
        Trazabilidad de fuentes & ¿La respuesta indica de qué fragmentos o metadatos se apoyó? & Fuentes recuperadas, puntaje de soporte y metadatos. & Cubierto parcialmente. \\
        Minimización de datos personales & ¿Se evita capturar identidad nominal del visitante cuando no es necesaria? & Identificador funcional de sesión y política de captura mínima. & Definido como criterio de diseño. \\
        Política de retención & ¿Existe una regla formal para conservar o eliminar sesiones, voz y logs? & Documento de consentimiento, retención y auditoría. & Pendiente para transferencia institucional. \\
        Endurecimiento de API & ¿La API tiene autenticación, límites y control de abuso en producción? & Autenticación, \textit{rate limiting}, logs y monitoreo. & Pendiente para producción. \\
        Pruebas de aceptación & ¿Cada módulo cuenta con criterio de aceptación antes del despliegue? & Protocolo de pruebas funcionales del Capítulo 9. & Cubierto a nivel de tesis. \\
        \hline
    \end{tabular}%
    }
    \caption[Matriz de verificación técnica mínima]{Matriz de verificación técnica mínima para contrastar la solución con los estándares aplicables. Fuente: elaboración propia.}
    \label{tab:matriz-verificacion-normativa}
\end{table}

Esta matriz permite distinguir tres niveles de madurez. El primero corresponde a elementos ya demostrados en el prototipo, como payload BLE, contrato HTTP/JSON y pruebas funcionales. El segundo corresponde a criterios definidos, pero todavía dependientes de una política institucional, como minimización y retención de datos. El tercero corresponde a requisitos de producción, como autenticación formal, límites de uso, monitoreo y auditoría.

\section{Documentación técnica y evidencia de aplicación}

La aplicación de estas normas y estándares no se apoya solo en declaraciones conceptuales. Puede rastrearse en artefactos concretos del proyecto. El contrato BLE descrito en el capítulo de datos de entrada documenta la estructura de proximidad y su correspondencia con identificadores de sala. El capítulo de diseño y despliegue muestra la interfaz HTTP/JSON, la separación por subsistemas, los fragmentos de código del backend y la lógica de integración entre capas. Los artefactos del módulo IoT BLE, la aplicación móvil y el backend RAG, además, funcionan como evidencia técnica distribuida de esta trazabilidad.
