...
\begin{figure}[!ht]
    ...
    \caption[Beacons basados en ESP32 para la solución propuesta]{Esquema visual del despliegue de beacons basados en ESP32 en salas y vitrinas del museo como infraestructura de proximidad de bajo costo. Fuente: elaboración propia con apoyo de generación asistida por IA.}
...
\subsection{Modelo RSSI--orientaci\'on para activaci\'on contextual}

El dise\~no de localizaci\'on interior de la soluci\'on propuesta se apoya en una conclusi\'on central del art\'iculo experimental desarrollado como antecedente del proyecto: RSSI es suficiente para estimar sala o sector probable, pero no siempre basta para decidir qu\'e obra espec\'ifica observa el visitante. Para ese segundo nivel de decisi\'on, la orientaci\'on del smartphone aporta una se\~nal complementaria porque permite contrastar la direcci\'on de observaci\'on con la ubicaci\'on relativa de vitrinas, paneles u obras dentro de la sala.

Desde el punto de vista computacional, la orientaci\'on no debe representarse como un valor angular lineal simple, porque 0 grados y 360 grados son direcciones equivalentes. Por ello, el modelo considera una representaci\'on circular:

\[
    o_i = [\sin(\theta_i), \cos(\theta_i)]
\]

donde \(\theta_i\) corresponde al rumbo estimado por el smartphone en el instante \(i\). Esta representaci\'on evita discontinuidades alrededor del norte magn\'etico y facilita combinar la orientaci\'on con observaciones RSSI suavizadas. Para activar una obra candidata \(o_k\), el puntaje contextual puede expresarse como:

\[
    S(o_k \mid i) = P(c=s_k \mid x_i^r) \cdot e^{\kappa \cos(\theta_i-\beta_k)}
\]

donde \(P(c=s_k \mid x_i^r)\) representa la probabilidad de que el usuario se encuentre en el sector \(s_k\) estimado con RSSI, \(\beta_k\) es la direcci\'on esperada hacia la obra y \(\kappa\) controla cu\'anto peso se asigna a la compatibilidad angular. As\'i, la soluci\'on no reemplaza BLE por orientaci\'on, sino que combina ambas se\~nales: BLE delimita el sector y la orientaci\'on prioriza la pieza m\'as coherente con el foco de observaci\'on.

\begin{table}[h!]
    \centering
    \footnotesize
    \setlength{\tabcolsep}{4pt}
    \begin{tabular}{>{\raggedright\arraybackslash}p{2.5cm} >{\centering\arraybackslash}p{2.4cm} >{\centering\arraybackslash}p{2.8cm} >{\centering\arraybackslash}p{2.6cm} >{\raggedright\arraybackslash}p{3.4cm}}
        \hline
        \textbf{Escenario} & \textbf{RSSI solo} & \textbf{RSSI + orientaci\'on} & \textbf{Mejora} & \textbf{Lectura para el dise\~no} \\
        \hline
        Potencia -8 dBm & 73.43\% & 84.57\% & +11.14 pp & La orientaci\'on incrementa discriminaci\'on entre obras cercanas \\
        Potencia -4 dBm & 71.00\% & 81.14\% & +10.14 pp & La mejora se mantiene aun con distinta potencia de emisi\'on \\
        Potencia 0 dBm & 67.00\% & 81.29\% & +14.29 pp & La orientaci\'on compensa parte de la ambig\"uedad de RSSI \\
        Promedio & 70.48\% & 82.33\% & +11.86 pp & El modelo h\'ibrido es preferible para activaci\'on contextual \\
        \hline
    \end{tabular}
    \caption[Resultados experimentales RSSI--orientaci\'on]{Resultados del escenario simulado de 7 sectores para comparar clasificaci\'on con RSSI solo frente a RSSI combinado con orientaci\'on. Fuente: elaboraci\'on propia a partir del art\'iculo experimental de localizaci\'on indoor.}
    \label{tab:resultados-rssi-orientacion}
\end{table}

\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.94\textwidth]{resultados_rssi_orientacion.png}
    \caption[Comparaci\'on de exactitud RSSI y RSSI con orientaci\'on]{Comparaci\'on visual de exactitud entre el enfoque basado solo en RSSI y el enfoque h\'ibrido RSSI--orientaci\'on. Fuente: elaboraci\'on propia.}
    \label{fig:resultados-rssi-orientacion}
\end{figure}

La Tabla~\ref{tab:resultados-rssi-orientacion} y la Figura~\ref{fig:resultados-rssi-orientacion} justifican que el dise\~no de la soluci\'on trabaje con dos niveles. El primero es el reconocimiento funcional de sala o sector usando BLE/RSSI. El segundo es la selecci\'on de obra probable usando orientaci\'on, inventario espacial y confirmaci\'on del usuario cuando la confianza no sea suficiente. Esta decisi\'on es especialmente adecuada para museos, donde el objetivo no es guiar al visitante como un sistema de navegaci\'on vehicular, sino entregar mediaci\'on cultural pertinente en el punto de observaci\'on.

\section{Tecnologías de interacción con el visitante}

\subsection{Smartphone como plataforma base BYOD}

La solución propuesta adopta un enfoque \textit{Bring Your Own Device} (BYOD), en el que el visitante utiliza su propio smartphone como plataforma de interacción. Desde el punto de vista tecnológico, esta decisión permite concentrar en un solo dispositivo las capacidades mínimas requeridas por el sistema: detección BLE, lectura de sensores, captura de voz, reproducción de audio y visualización de contenido. De este modo, la aplicación puede operar sobre una plataforma ya disponible para el usuario sin requerir terminales especializados por parte del museo.
...
\begin{figure}[!ht]
    ...
    \caption[Smartphone como plataforma BYOD en la solución propuesta]{Representación del enfoque Bring Your Own Device, en el que el smartphone del visitante concentra las funciones principales de interacción con el sistema. Fuente: elaboración propia con apoyo de generación asistida por IA.}
...
Una limitación crítica de los modelos de lenguaje es la posibilidad de producir respuestas plausibles pero incorrectas o no verificables. En dominios patrimoniales, esta limitación es especialmente sensible, ya que el contenido debe respetar criterios curatoriales, trazabilidad institucional y fidelidad interpretativa. Por ello, la solución propuesta incorpora Retrieval-Augmented Generation (RAG) como tecnología base para la capa inteligente. El enfoque RAG combina recuperación de información desde una memoria externa con generación condicionada, de modo que la respuesta se apoye en fragmentos documentales recuperados del corpus pertinente~\cite{lewis2020rag}.

La relevancia de RAG para la solución propuesta es directa: permite anclar la conversación a fichas, descripciones curatoriales, textos de sala o documentación institucional, reduciendo alucinaciones y mejorando consistencia semántica. Además, trabajos recientes sobre reducción de alucinación mediante recuperación aumentada refuerzan la utilidad de este paradigma en sistemas que requieren salidas más controladas y estructuradas~\cite{bechard2024hallucination}. En consecuencia, la inteligencia de la solución propuesta no se concibe como una generación abierta, sino como una conversación guiada por evidencia documental recuperada.

\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.92\textwidth]{src/imgs/07/rag-museiq.png}
    \caption[Recuperación aumentada de información en la solución propuesta]{Esquema conceptual del uso de RAG para recuperar fragmentos curatoriales relevantes antes de generar una respuesta al visitante. Fuente: elaboración propia con apoyo de generación asistida por IA.}
...
\subsection{Ventajas y riesgos de la inteligencia artificial en la solución propuesta}

La incorporación de IA ofrece ventajas claras para la solución propuesta: mayor flexibilidad conversacional, posibilidad de responder consultas abiertas, adaptación a distintas formas de preguntar y mejora de la experiencia interpretativa. No obstante, también introduce riesgos. El más importante es la alucinación, es decir, la generación de respuestas plausibles pero incorrectas. También deben considerarse problemas de gobernanza del contenido, control de fuentes y coherencia con narrativas institucionales.

Por estas razones, la elección tecnológica no puede justificarse únicamente por novedad o capacidad generativa. Su valor para la solución propuesta depende de estar acotada por RAG, por un corpus institucional bien definido y por políticas de respuesta compatibles con un dominio patrimonial. En este punto, la tecnología no reemplaza la curaduría, sino que la amplifica mediante una capa de acceso conversacional y contextual.

\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.92\textwidth]{src/imgs/07/ventajas-riesgos-ia.png}
    \caption[Ventajas y riesgos de la inteligencia artificial en la solución propuesta]{Composición comparativa de beneficios y riesgos asociados al uso de inteligencia artificial en una guía museística contextual y conversacional. Fuente: elaboración propia con apoyo de generación asistida por IA.}
...
En consecuencia, este capítulo no agota el detalle ingenieril de la solución, pero sí establece con claridad qué tecnologías sostienen la solución propuesta y por qué su combinación resulta defendible dentro de una tesis de ingeniería aplicada orientada al contexto museístico peruano.
